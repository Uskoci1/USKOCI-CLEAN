> **HISTORICAL (PKG-012, 2026-09-16).** This document records an earlier checkpoint. The current entry map is `execution/CURRENT_ENTRY_MAP_20260916.md` (docs/implementation/execution/CURRENT_ENTRY_MAP_20260916.md); package state lives in `execution/PACKAGE_RECONCILIATION_20260916.md` and `execution/EXECUTION_LEDGER.jsonl`.
...
W05 [PR91](https://github.com/Uskoci1/USKOCI-CLEAN/pull/91) merged as `eb125177050d77be8ea5192d1fb83d6b94e54e04` with all 13 exact-head checks successful. Its actual14-group DB proof at source103 includes six55P03 lock results; original artifact10161277307 SHA256 b8d864273bd3ea505d5f7e6b4e1bc8564ce80a1b1130fffa3a294c061cd9f82e is verified. SQL103 and Edge v1 are promoted with exact readback. Source104 admits every migration before retaining original D0140 assertions at102 and original P2 intake assertions at103; the intentional extensions are separately proved. P3 compares every original row/column and explicitly validates only the added NULL export_delivery column.

Next: run and inspect the P2 real Storage proof, then integrate the coherent delivery batch. Production is accepted LIVE103; P2 source104 remains separate and unpromoted. Do not reapply SQL103. RC2/Retention V1 are approved but their exact content is still inaccessible. Recurring Edge worker credentials, account closure, complete media export and physical native save remain dependencies. Continue P3 execution and the remaining master plan after this batch.

---

## Current W05 checkpoint — 2026-09-10 / canonical and LIVE103

W05 publication is **UI_CONNECTED / MERGED / DISPOSABLE_PROVEN / LIVE_STRUCTURAL** at `eb125177050d77be8ea5192d1fb83d6b94e54e04`. R04 performs an explicit saved-Need/revision check, then a separate user-confirmed canonical publication with no additional application deadline. Only a validated stored ALLOW enables publication. Unknown outcomes keep the same key until readback; known rejections require fresh evaluation. Existing R07 supports correction through an actual OPEN conversation bound to DRAFT. Account, focus, revision and context guards retire stale callbacks.

[Exact-head PRE-P4](https://github.com/Uskoci1/USKOCI-CLEAN/actions/runs/34498940991) and [CodeQL](https://github.com/Uskoci1/USKOCI-CLEAN/actions/runs/34498937338) passed for `3b50eaaa64ec630a4812e862ea759993cf43f41d`; its tested merge and canonical commit share tree `8dab583ec04fa8f9ac4a2478e1ff1ceced77ef42`. Original evidence proves 14 W05 DB groups, 11 location cases, six distinct locked writes and preserved predecessor102 invariants. Edge31 uses synthetic transports. All three original CodeQL analyses/SARIF have zero findings, errors and warnings.

Production SQL103 is `20260910162450_clean_w05_publication_evaluator_authority`, applied once with all 67,989 source bytes preserved: MD5 `d202ef982a457dc396e6914d0761aa29`, SHA256 `0fb44625a7520c0fecfba38f34b326c057020dae5226798e1d43e8f79e045057`. Read-only postflight at 16:25:14 UTC matched 14 function bodies, kept five helpers private and context authenticated-only, and retained all16 pre/post comparisons including original102 history and 65 business/security table aggregates. Private fingerprint/context execution was not observed; catalog and aggregate checks establish this live boundary.

`uskoci-publication-evaluate` is ACTIVE v1/JWT true, with exact 25,786-byte source readback, SHA256 `6b56e4106d373b86bf710a9434bd3d0da572e7aff0fb9b6e73adddd1d709f9a2`. [The compact promotion evidence](evidence/w05-live103-20260910/verification.json) binds original CI and live receipts. Policy stays unreviewed/incomplete/inactive; no executable legal content was seeded or activated. Approved source-content reconciliation, authenticated provider classification and native publication journey remain **DEFERRED / NOT_OBSERVED**. Passing Android checks cover Auth recovery on an emulator with disposable GoTrue/Mailpit only.

Continue existing P2 delivery without replaying closed migrations. Canonical/live103 and working source104 are distinct; frozen historical87 replay metadata retains its original meaning. RS BUILDING, 0 RSD/checkout OFF and the unresolved authenticated LocationIQ/native pin proof remain. The preceding PR90/live102 and older checkpoints below retain their historical evidence.
...
All seven original ENTRY PNGs and the sampled original video were reviewed. The video exposes the old blue Expo splash and only about 3.5 seconds of the 4.5-second owner sequence; approximately one second hidden behind native splash is an inference, not instrumented timing. The source now mounts the SVG scene paused at its first frame, waits for positive layout, completed hide request and two frame callbacks, then starts the unchanged 4500ms clock. Native splash is warm with a transparent Android drawable. Readiness failure skips cosmetic motion; storage, unmount, reduced motion and Back remain bounded. Fresh original Android video must prove visible timing; no 60fps or full registration/recovery claim is made.

Validation: integrated **61 suites / 540 tests**, TypeScript, 21 selector tests, and AST **78 client files / 30 presentation files / 0 findings** PASS. The AST includes per-file source fingerprints at this dirty merge boundary: `evidence/spoj-entry-20260907/replay-source/client-architecture.json`. The existing native workflow records exact commit, APK and driver hashes. Final CI/CodeQL and full fresh native review remain required. See `SPOJ_ENTRY_IMPLEMENTATION_20260907.md`. This checkpoint performs no production action or new live observation and closes none of the remaining 47-item backlog by appearance. Older checkpoints below retain their original boundaries.
...
- pre-deletion old-vs-new equivalence: GitHub Actions `33957532925` — PASS
- final post-deletion proof: GitHub Actions `33957660880` — PASS (migration integrity, TypeScript, full regression)
...
- RU-4B remains **LIVE_FOUNDATION / ACTIVATION_BLOCKED**
- production D0140 publication remains **FAIL_CLOSED**
- exact next cursor: **FRESH POST-A05 SHADOW INVENTORY → SELECT CDL-A06 BY LOWEST-RISK PROVEN SLICE**
