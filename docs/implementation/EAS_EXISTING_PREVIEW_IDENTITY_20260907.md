...
The metadata snapshot contains configured FCM V1 service-account metadata referencing `uskoci-ed59b`; it does not establish working Firebase client configuration, token registration or delivery. The parent's browser inspection of the current Google account could not list the Firebase project's apps (project absent or insufficient access). The public client `google-services.json` was unavailable at this checkpoint. EAS project environment metadata was empty. No EAS environment variables were written by this unit.
