...
### Native R19 symbol failure — bounded correction, 2026-09-26

The integrating session's `outputs/r6-integration/r19-emulator/discovery-first.png` shows blank native disks **and no base-map place labels** on source `3cdb3005`. The two larger disks match native clusters (radius 20); the smaller one matches the unclustered fallback (radius 16). These are different render paths. This fresh R19 observation supersedes any implication that the annotation refresh alone resolved the visible problem.

One concrete client/SDK boundary is corrected: the dynamic cluster `Layer` now uses `text-font: ['literal', ['Noto Sans Regular']]`. Installed RN SDK 11.3.10's Android `MLRNStyleValue.kt:54–63` treats string-first arrays as expressions, and `MLRNStyleFactory.kt:1691` forwards that expression. The installed real style-expression parser rejects the old bare font array as an unknown expression and accepts/evaluates the explicit literal. The new regression exercises the actual rendered prop with that parser, not only a mocked native layer. This is a parser/bridge correction, not proof that every missing native symbol is fixed.

The base style passes directly as JSON and does not cross the dynamic Layer prop bridge. Both the current public Positron style and the result of the actual `latinLabels(uskociMapColors(...))` passed installed style-spec validation, with the glyph URL preserved. The Noto Sans Regular `0-255.pbf` endpoint returned HTTP 200 (76,580 bytes) from this computer. Base-style fonts and label expressions are therefore unchanged in this correction.

The fallback PNG loads through `MLRNImages`: an empty placeholder is registered before asynchronous bitmap loading. A loading failure could leave a ring, but none was established here. The asset is 640×640, matching the current icon scale. SDK source inserts queued source children in ascending order and places each symbol after its disk; no reversed default layer ordering was found. No fallback, renderer, dependency, or base-style workaround was applied on speculation.

The upstream [MapLibre Native issue #3648](https://github.com/maplibre/maplibre-native/issues/3648) reports missing OpenFreeMap labels and symbols on Android OpenGL emulators while physical-device rendering works. This is relevant corroboration, **not proof** that USKOČI has the identical driver cause. The root session owns emulator/physical-device comparison and any renderer diagnosis. Native acceptance remains open.

Focused checks after the font correction: **4 suites / 84 tests passed** (`discovery-map-pills`, `discovery-map`, `map-style`, `map-appearance`); `npx tsc --noEmit -p tsconfig.json` and the scoped `git diff --check` passed. No server, provider, dependency or device operation belongs to this correction.

### Same-APK graphics comparison supersedes the open symbol hypothesis

The integrating agent restarted the same USKOCI_V5_TEST AVD using host graphics, no snapshot load, and no app-data wipe. APK SHA `a9328dc6…` remained unchanged. `discovery-host-renderer`, `discovery-host-novi-sad`, and `discovery-host-selected` show the actual place labels, counts, native fallback mark and rich logo/Ponude annotation. See NATIVE_REVIEW.md and CAPTURES.emulator.first.json. Therefore the later font-literal change is compatibility normalization, **not the demonstrated cause or cure of the blank rings**. The first APK already renders the cluster count after the graphics restart. No production renderer workaround is justified by this emulator result; physical-phone acceptance remains separate.

### Re-entry after task navigation — native follow-up

The first R19 APK lost the entire list sheet after a fast collapse → open task → Back sequence, while the parent map/filters remained usable. The source kept one native sheet spring across route detachment without a focus re-entry boundary. The correction mounts the native sheet once on re-entry (not on blur) at the remembered requested stop; map, filter, selected pin and scroll memory remain above that boundary. It does not change queries or commands.

Before/after regressions demonstrate three concrete stale-callback failures: an old completion changed requested peek 0 back to full 2, another cleared a retained selected pin, and an old pending scroll overwrote saved offset 160 with 260 after blur. The captured visit now guards index, scroll, content-size and begin-drag deliveries; cleanup retires the delayed save. Explicit pre-navigation offset flushing remains. Refocus restores the saved offset immediately and again after content layout when necessary. **68 focused Discovery tests and TypeScript pass.** Independent review confirmed the scroll race correction and no remaining blocking finding in this bounded diff; it did not rerun the root checks. Exact rebuilt-APK replay remains separate from these unit results. No exact internal Gorhom cause beyond the observed disappearance and evidenced lifecycle gap is asserted.
