...
## Gallery scaling boundary observed during integration

`mediaClientService.readNeedPhotos` rejects more than six photos, and the native picker prepares JPEGs with a longest edge of 1,600 pixels. This is a bounded task gallery, not an unbounded history. Nevertheless, `PhotoPages` currently mounts every photo; opening the full-screen modal mounts a second set while the inline set remains hidden for accessibility. `AuthorizedPhoto` has contextual, nonpersistent reads rather than a shared persistent image cache. Therefore the modal can repeat up to six authorized reads and hold both representations until it closes. No native memory or transfer benchmark was run here. Current/adjacent-page mounting and in-scope reuse are a future optimization if device measurements justify them; this does not justify a persistent private-image cache or relaxed account retirement.
