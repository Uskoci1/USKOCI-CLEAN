...
## Native findings and their separate outcomes

1. Historical, resolved on the same first APK: `discovery-first` and `discovery-cluster-zoom` showed blank disks and no base-map labels. A normal restart of the same AVD using host graphics restored labels, cluster counts and logos; see the isolation section below. The font literal correction is separate native-bridge normalization, not the demonstrated cause or cure of that global blank-symbol observation. Physical-phone rendering still needs its own check.
...
Five later first-APK captures at approximately 22:36–22:37 UTC inspect the existing internal AI gallery: selector, task welcome, worker welcome, a tapped worker starter with keyboard, and the ready-task fixture. Both welcomes show the real brand and three relevant starting prompts. Tapping a worker starter puts text in the editable composer; no Send or microphone was used. The lower welcome choices scroll below the composer when the keyboard opens. Despite its filename, `ai-task-thread` is the `ready` scene (Montaža police/OFFERS); the manifest records the observed scene. Those canned gallery messages are not a live AI response and cannot prove category extraction or interview quality. One transient UI dump failed before the successful worker welcome capture; it produced no saved image.
