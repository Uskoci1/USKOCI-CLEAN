"""Admit the saved private Agreement photo144 proof to the existing disposable runner."""
import argparse
import base64
import difflib
import json
...
assert config['finalHistoryCount'] == 143 and len(config['proofs']) == 27
...
migration = '20260913065130_clean_v5_agreement_private_photos.sql'
proof = 'v5_agreement_photos_proof.mjs'
report = 'v5-agreement-photos-report.json'
...
config.update(sourceSha=args.source, sourceTree=source['tree']['sha'], finalHistoryCount=144,
              purpose='Exact saved V5 source: full regression and disposable Auth/Postgres/Storage proofs through private Agreement photographs144; no source mutation, live access, provider calls, device, deployment or release.')
config['proofs'].append({'script': proof, 'report': report})
inventory = before[paths[1]]
anchor = "'20260913045824_clean_v5_support_case_authority.sql')"
...
anchor = "'v5_support_case_proof.mjs')"
...
(out / 'extend144-control-update.diff').write_text(diff, encoding='utf-8', newline='\n')
receipt = {'previousControlCommit': args.previous_control, 'previousSourceCommit': previous_source,
           ...
           'fullRegression': True, 'expectedHistoryCount': 144, 'expectedSqlReports': 34,
...
if args.apply:
    ...
    commit = api(f'{repo}/git/commits', {'message': 'test(v5): admit exact private Agreement photograph proof144',
...
(out / 'extend144-control-update-receipt.json').write_text(json.dumps(receipt, indent=2) + '\n', encoding='utf-8', newline='\n')
