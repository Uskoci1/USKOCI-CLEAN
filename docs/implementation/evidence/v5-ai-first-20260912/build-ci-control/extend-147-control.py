"""Admit the saved account-erasure146 and approved Q&A147 proofs to the existing disposable runner."""
import argparse
import base64
import difflib
import json
...
assert config['finalHistoryCount'] == 145 and len(config['proofs']) == 29
...
additions = [
    ('20260913081147_clean_v5_event_bound_account_erasure.sql', 'v5_account_erasure_proof.mjs', 'v5-account-erasure-report.json'),
    ('20260913081242_clean_v5_qa_owner_product_activation.sql', 'v5_qa_owner_activation_proof.mjs', 'v5-qa-owner-activation-report.json'),
]
for migration, proof, report in additions:
...
config.update(sourceSha=args.source, sourceTree=source['tree']['sha'], finalHistoryCount=147,
              purpose='Exact saved V5 source: full regression and disposable Auth/Postgres/Storage proofs through event-bound account erasure146 and approved Q&A147; no source mutation, live access, provider calls, device, deployment or release.')
config['proofs'].extend({'script': proof, 'report': report} for migration, proof, report in additions)
inventory = before[paths[1]]
anchor = "'20260913080237_clean_v5_self_reported_identity_requirement.sql')"
assert inventory.count(anchor) == 1 and all(migration not in inventory for migration, _, _ in additions)
inventory = inventory.replace(anchor, anchor[:-1] + ',' + ','.join(repr(migration) for migration, _, _ in additions) + ')')
checks = before[paths[2]]
anchor = "'v5_self_reported_identity_proof.mjs')"
assert checks.count(anchor) == 1 and all(proof not in checks for _, proof, _ in additions)
checks = checks.replace(anchor, anchor[:-1] + ',' + ','.join(repr(proof) for _, proof, _ in additions) + ')')
...
(out / 'extend147-control-update.diff').write_text(diff, encoding='utf-8', newline='\n')
receipt = {'previousControlCommit': args.previous_control, 'previousSourceCommit': previous_source,
           ...
           'fullRegression': True, 'expectedHistoryCount': 147, 'expectedSqlReports': 37,
...
if args.apply:
    ...
    commit = api(f'{repo}/git/commits', {'message': 'test(v5): admit exact account erasure146 and approved QA147 proofs',
...
(out / 'extend147-control-update-receipt.json').write_text(json.dumps(receipt, indent=2) + '\n', encoding='utf-8', newline='\n')
