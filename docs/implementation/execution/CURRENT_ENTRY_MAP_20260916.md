...
| `src/app/(app)/pregled-nacrta.tsx` | Hidden tab route, reachable by URL. Since 2026-09-18 it is an explicit `Redirect` to `/pregled-zadatka` carrying its `conversationId`, after the screen body was deleted without authority and restored as this shim. | Retirement candidate for PKG-023, after parity with `/pregled-zadatka` and owner approval. The parity gap named on 2026-09-18 was that no client path saved a draft without asking for publication; since 2026-09-19 the review's "Sačuvaj nacrt" does (`aiTaskReviewClientService.acceptAsDraft`: accept the displayed review, stop before evaluation). Owner approval is still outstanding, so the route stays. Resolves explicitly today. |
