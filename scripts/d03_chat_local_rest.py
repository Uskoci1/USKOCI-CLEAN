...
def validate_local_targets(env):
    ...
    if not api_ok or not db_ok:
        raise RuntimeError('D03 chat proof requires exact disposable loopback targets')
    if env.get('RU5_DEVICE_PACKAGE') != 'rs.uskoci.n04proof' or env.get('RU5_DEVICE_PROOF_DIR') != PROOF_DIR:
        raise RuntimeError('D03 chat proof package/directory mismatch')
    if env.get('DOCKER_HOST', '') not in ('', DOCKER_SOCKET) or env.get('DOCKER_CONTEXT', '') not in ('', 'default'):
        raise RuntimeError('D03 chat outage requires the local default Docker daemon')
...
class LocalRestOutage:
    ...
    def stopped(self):
        ...
        try:
            ...
            print(f'CHECKPOINT D03_LOCAL_REST_STOPPED id={self.container_id}', flush=True)
            yield
        finally:
            ...
            validate_container(self.inspect(self.container_id), expected_running=True, expected_id=self.container_id)
            self.wait_http(available=True)
            print(f'CHECKPOINT D03_LOCAL_REST_RESTORED id={self.container_id}', flush=True)
