...
# Retain the original 10 Inbox +17 navigation observations before the new slice.
bash scripts/intent_shell_run_journey.sh
status=$?
cp "$artifact_dir/proof.log" "$artifact_dir/shared-discovery-proof.log"
copy_status=$?
...
if [[ "$status" -eq 0 ]]; then
  node <<'NODE' >> "$artifact_dir/shared-discovery-proof.log" 2>&1
...
const frozen = JSON.parse(fs.readFileSync(path.join(dir, 'shared-discovery-source.json'), 'utf8'));
...
originalApp.expo.name = 'USKOČI DISCOVERY PROOF';
...
fs.writeFileSync(path.join(dir, 'shared-discovery-build.json'), JSON.stringify(report, null, 2) + '\n');
console.log('PASS SHARED_DISCOVERY_BUILD exact_source canonical_prebuild_inputs explicit_generated_overrides actual_apk_hash');
NODE
  status=$?
fi
...
seeded=0
if [[ "$status" -eq 0 ]]; then
  node scripts/shared_discovery_fixture.mjs seed >> "$artifact_dir/shared-discovery-proof.log" 2>&1
  status=$?
  if [[ "$status" -eq 0 ]]; then seeded=1; fi
fi
if [[ "$status" -eq 0 ]]; then
  python3 scripts/shared_discovery_android_journey.py >> "$artifact_dir/shared-discovery-proof.log" 2>&1
  status=$?
fi
# A failed UI journey must still expose unexpected business effects after seed.
if [[ "$seeded" -eq 1 ]]; then
  node scripts/shared_discovery_fixture.mjs postflight >> "$artifact_dir/shared-discovery-proof.log" 2>&1
  postflight_status=$?
  if [[ "$status" -eq 0 ]]; then status=$postflight_status; fi
fi
if [[ "$status" -eq 0 ]]; then
  python3 scripts/shared_discovery_validate.py >> "$artifact_dir/shared-discovery-proof.log" 2>&1
  status=$?
fi
if [[ "$status" -ne 0 ]]; then
  timeout 20s adb exec-out screencap -p > "$artifact_dir/DISCOVERY_FAILURE_last_screen.png" 2>/dev/null || true
  timeout 20s adb shell uiautomator dump /sdcard/shared-discovery-failure.xml >/dev/null 2>&1 || true
  timeout 20s adb shell cat /sdcard/shared-discovery-failure.xml > "$artifact_dir/DISCOVERY_FAILURE_last_screen.xml" 2>/dev/null || true
fi
cat "$artifact_dir/shared-discovery-proof.log"
exit "$status"
