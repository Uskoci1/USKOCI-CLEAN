...
describe('the status line', () => {
  ...
  it('a chosen application reads green and a waiting one warns with the orange dot; one that is over is quiet', async () => {
    ...
    expect(style(textNode('Potrebna nova provera')).color).toBe(sys.color.warn);
    expect(style(tree.root.findByProps({ testID: 'card-status-dot' })).backgroundColor).toBe(sys.color.orange);
...
describe('the one foot action', () => {
  const CASES: [string, MojaPrijavaProjekcija, boolean, string | null][] = [
    ...
    ['waiting for a new check', inState('STALE_REVIEW_REQUIRED'), false, 'Pregledaj izmene: Unos ormara'],
  ...
  it('is a quiet row link on the card\'s white (the waiting one on the wash strip), with 48 px of touch and no fill of its own', async () => {
    ...
    expect(style(textNode('Povuci prijavu')).color).toBe(sys.color.danger);
    await render(<ApplicationCard row={inState('SELECTED')} {...handlers()} />);
