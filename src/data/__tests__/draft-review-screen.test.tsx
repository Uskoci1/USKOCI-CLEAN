...
const reveal = async (label = 'Naslov') => { await act(async () => tree.root.findByProps({ accessibilityLabel: `Pregledaj: ${label}` }).props.onPress()); };
...
describe('owned parent draft review screen', () => {
  it.each([null, 'need-a'])('requires human confirmation of optional proposals before saving a %s review', async bound => {
    ...
    await render(); const save = button(bound ? 'Sačuvaj izmene' : 'Sačuvaj nacrt');
    ...
    expect(text()).toContain('Pregledaj i preostale predloge');
  });
  ...
  it('edits an actual OPEN conversation bound to a saved DRAFT through the existing confirmEdit owner', async () => {
    ...
    await act(async () => { await button('Sačuvaj izmene').props.onPress(); });
  ...
  it.each(['COMPLETED', 'ABANDONED', undefined, 'UNKNOWN'])('does not infer an edit from boundNeedId when status is %s', async status => {
    ...
    expect(tree.root.findAllByProps({ label: 'Sačuvaj izmene' })).toHaveLength(0);
  ...
  it('refreshes on return from the real location editor and rejects a stale review action', async () => {
    ...
    await render();
    const oldSave = button('Sačuvaj nacrt').props.onPress;
  ...
  it('serializes draft double taps and navigates once using the accepted receipt', async () => {
    ...
    await render();
    const save = button('Sačuvaj nacrt').props.onPress;
  ...
  it.each(['account', 'blur'] as const)('cannot navigate after a pending save loses ownership by %s', async change => {
    const pending = deferred<unknown>(); mockSave.mockReturnValueOnce(pending.promise);
    await render();
    await act(async () => { void button('Sačuvaj nacrt').props.onPress(); });
  ...
  it('makes an unknown write outcome readable and requires readback before another send', async () => {
    ...
    await render(); const retained = button('Sačuvaj nacrt').props.onPress;
    await act(async () => { await retained(); });
    expect(text()).toContain('Čuvanje nije potvrđeno'); expect(text()).not.toContain('PostgreSQL');
    expect(button('Sačuvaj nacrt').props.disabled).toBe(true);
    await act(async () => { await retained(); }); expect(mockSave).toHaveBeenCalledTimes(1);
    await act(async () => { await button('Učitaj pregled ponovo').props.onPress(); });
    await act(async () => { await button('Sačuvaj nacrt').props.onPress(); });
  ...
  it('confirms the exact visible Need revision and never silently adopts the latest server revision', async () => {
    ...
    mockConfirmEdit.mockResolvedValueOnce({ ok: false, kod: 'NEED_VERSION_CONFLICT', poruka: 'Zadatak je promenjen. Učitaj pregled ponovo.' });
    await render();
    await act(async () => { await button('Sačuvaj izmene').props.onPress(); });
    ...
    expect(button('Sačuvaj izmene').props.disabled).toBe(true);
  });
  ...
  it('does not present an already-saved draft while a bound public Need read is still pending', async () => {
    ...
    expect(button('Sačuvaj izmene')).toBeDefined();
  });
  ...
  it('serializes fact confirmations and only displays their fresh authoritative review', async () => {
    ...
    await render(); await reveal(); const confirm = button('Potvrdi').props.onPress;
  ...
  it('blocks retained local edit/navigation actions after account changes before rerender', async () => {
    await render(); await reveal(); const open = button('Mesto Zadatka').props.onPress, edit = button('Izmeni').props.onPress;
    mockSession = { user: { id: 'account-b' }, accountRevision: 2, sessionEpoch: 2 };
  ...
  it('serializes corrections and does not reload their late result after leaving the screen', async () => {
    ...
    await render();
    await reveal();
    await act(async () => button('Izmeni').props.onPress());
  ...
  it('shows one focused fact editor and keeps private evidence behind its row', async () => {
    ...
    await reveal(); expect(button('Izmeni')).toBeDefined();
    await reveal('Pristup'); expect(tree.root.findAllByProps({ label: 'Izmeni' })).toHaveLength(0);
  ...
  it('does not save the old value while a focused correction is unfinished', async () => {
    await render(); await reveal(); await act(async () => button('Izmeni').props.onPress());
    expect(button('Sačuvaj nacrt').props.disabled).toBe(true);
    await act(async () => button('Sačuvaj nacrt').props.onPress()); expect(mockSave).not.toHaveBeenCalled();
  });
  ...
  it('retains an unfinished correction when another row or a retained mutation is pressed', async () => {
    ...
    const oldConfirm = button('Potvrdi').props.onPress, oldLocation = button('Mesto Zadatka').props.onPress;
    const oldSave = button('Sačuvaj nacrt').props.onPress;
    const otherRow = tree.root.findByProps({ accessibilityLabel: 'Pregledaj: Opis' }).props.onPress;
    await act(async () => button('Izmeni').props.onPress());
    ...
    await reveal('Opis'); expect(tree.root.findByProps({ accessibilityLabel: 'Pregledaj: Opis' }).props.accessibilityState.expanded).toBe(true);
  });
