...
test('header names the intent the user is in, the screen title is a header, and the mode switch is a real tab list', async () => {
  ...
  // carried one name. Discovery is titled Prilike: "Mapa" was the tab label, the screen title
  ...
  expect(texts()).toContain('Ja mogu'); expect(texts()).toContain('Prilike');
  expect(tree.root.findAllByType('T' as React.ElementType).some(node => node.props.accessibilityRole === 'header' && node.children.includes('Prilike'))).toBe(true);
