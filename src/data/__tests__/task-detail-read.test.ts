...
const row = () => ({
  ...
  requester_profile_id: 'requester-a', mode: 'MY_PRICE', requester_price_rsd: 5000,
});
...
describe('W04 public-safe detail read', () => {
  ...
  it('recovers with the existing narrow projection and no private task columns', async () => {
    ...
    expect(select.mock.calls[0][0].split(',').map((field: string) => field.trim())).toEqual([
      ...
      'required_slots', 'required_skills', 'required_tools', 'required_vehicles', 'covered_slots', 'mode', 'requester_price_rsd', 'requester_profile_id',
    ]);
