...
jest.mock('../supabaseClient', () => ({ supabaseKlijent: () => ({ auth: { getUser: mockGetUser }, from: mockFrom }) }));
...
const row = { id: P, account_id: A, kind: 'WORKER', display_name: 'Ana', city: 'Novi Sad', bio: '', skills: ['Prevoz'],
  tools: [], vehicles: ['Kombi'], profile_status: 'ACTIVE', available_now: true, radius_km: 20 };
beforeEach(() => {
...
it('reads only the captured authenticated Worker projection, preserving explicit availability and radius', async () => {
  ...
  expect(profile).toEqual({ id: P, ime: 'Ana', grad: 'Novi Sad', biografija: '', vestine: ['Prevoz'], alati: [], vozila: ['Kombi'],
    stanje: 'ACTIVE', dostupanOdmah: true, radijusKm: 20 });
  expect(mockFrom).toHaveBeenCalledWith('app_profiles');
  expect(mockSelect).toHaveBeenCalledWith('id,account_id,kind,display_name,city,bio,skills,tools,vehicles,profile_status,available_now,radius_km');
  expect(mockEq.mock.calls).toEqual([['account_id', A], ['kind', 'WORKER']]);
});
it('only successful missing row is absence; Auth and table errors cannot create a default profile', async () => {
  ...
  mockFrom.mockClear(); mockGetUser.mockResolvedValue({ data: { user: null }, error: { message: 'private auth diagnostic' } });
...
it.each([
  { account_id: B }, { id: 'invalid' }, { kind: 'REQUESTER' }, { profile_status: 'UNKNOWN' }, { radius_km: null },
...
it.each(['auth', 'select'])('fences account ABA after an asynchronous %s operation', async stage => {
  ...
  await rejected; if (stage === 'auth') expect(mockFrom).not.toHaveBeenCalled();
});
