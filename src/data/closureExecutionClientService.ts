...
export type ClosureExecutionReview={accountId:string;requestId:string|null;revision:number;ready:boolean;policySha256:string|null;blockers:string[];code:string|null;retainedDatasets:RetainedClosureDataset[]|null;authoritative:true};
...
export type ClosureExecutionState={accountId:string;requestId:string;generation:string;state:'EXECUTING'|'CLOSED';policySha256:string;authoritative:true;closedAt?:string;retainedDatasets?:RetainedClosureDataset[]};
...
export const closureExecutionClientService={
 review(explicit?:ReceiptAccount):Promise<Ishod<ClosureExecutionReview>>{
  const a=owner(explicit);if(!a)return bad();return readOwnedResult({account:a,errors,fallback:'CLOSURE_READ_UNAVAILABLE',invalid:'CLOSURE_INVALID_RESPONSE',request:()=>supabaseKlijent().rpc('rpc_review_account_closure_execution',{p_expected_user_id:a.accountId}),decode:raw=>{
   const r=record(raw);if(!r||!sameId(r.accountId,a.accountId)||(r.requestId!==null&&!uuid(r.requestId))||!revision(r.revision)||typeof r.ready!=='boolean'||(r.policySha256!==null&&!hash(r.policySha256))||!Array.isArray(r.blockers)||r.blockers.some(x=>typeof x!=='string'||!closureBlockerLabels[x])||new Set(r.blockers).size!==r.blockers.length||r.authoritative!==true||r.authAction!=='AUTH_IDENTITY_ERASED_SUBJECT_RETAINED'||r.mediaAction!=='DELETE_OWNED_OBJECTS'||(r.code!==null&&!['CLOSURE_POLICY_NOT_READY','CLOSURE_PREPARATION_REQUIRED','CLOSURE_BLOCKED'].includes(String(r.code))))return null;
   const datasets=r.retainedDatasets===null?null:retainedDatasets(r.retainedDatasets);if(r.retainedDatasets!==null&&!datasets||r.ready&&(!r.requestId||!r.policySha256||r.revision===0||r.blockers.length>0||r.code!==null||!datasets))return null;
   return {accountId:a.accountId,requestId:r.requestId as string|null,revision:r.revision,ready:r.ready,policySha256:r.policySha256 as string|null,blockers:r.blockers as string[],code:r.code as string|null,retainedDatasets:datasets,authoritative:true};}});
 },
