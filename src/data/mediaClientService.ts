...
export const mediaClientService={
  ...
  async readMedia(assetId:string,context:{needId?:string;profileId?:string}={},options:MediaRequestOptions={}):Promise<Ishod<MediaImage>>{
    const owner=sesijaSada();if(!owner.user?.id)return failure('AUTH_REQUIRED',errors.AUTH_REQUIRED);
    const account={accountId:owner.user.id,accountRevision:owner.accountRevision};
    if(!uuid(assetId)||(context.needId&&!uuid(context.needId))||(context.profileId&&!uuid(context.profileId))||(context.needId&&context.profileId))return failure('MEDIA_NOT_FOUND',errors.MEDIA_NOT_FOUND);
    return readOwnedResult({account,errors,fallback:'MEDIA_UNAVAILABLE',invalid:'MEDIA_INVALID_RESPONSE',request:async()=>{
      const result=await supabaseKlijent().functions.invoke('uskoci-media',{body:{assetId,...context},headers:{'x-media-operation':'read'},signal:options.signal});
