...
export function AgreementActionsPresentation(p: AgreementActionsPresentationProps) {
  ...
  if (form) {
    ...
    if (p.phase === 'LOADING' && !snapshot) {
      // TODO(r6 row 3, Skeleton.tsx): pass `variant: 'facts'` once Skeleton exports it; what arrives here is a flat list
      // of facts, not two cards.
      body = <StateView kind="loading" title="Učitavamo važeće uslove…" skeleton={{ count: 2, rows: 3 }} />;
