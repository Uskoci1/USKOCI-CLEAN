...
// Deep read 8.18: the start asks once more before the irreversible step.
const confirmStart=async()=>{await act(async()=>button('Pokreni zatvaranje naloga').props.onPress());};
...
it('the first tap only asks; cancelling leaves nothing started',async()=>{await render();await confirmStart();expect(mockSave).not.toHaveBeenCalled();expect(mockStart).not.toHaveBeenCalled();await act(async()=>button('Odustani').props.onPress());expect(button('Pokreni zatvaranje naloga')).toBeDefined();expect(button('Da, trajno zatvori nalog')).toBeUndefined();expect(mockStart).not.toHaveBeenCalled();});
it('cannot submit after local durable storage failure',async()=>{mockSave.mockRejectedValue(new Error('unavailable'));await render();await confirmStart();await act(async()=>button('Da, trajno zatvori nalog').props.onPress());expect(mockStart).not.toHaveBeenCalled();});
it('account ABA during key persistence fences the network write',async()=>{let resolve!:()=>void;mockSave.mockReturnValue(new Promise<void>(r=>{resolve=r;}));await render();await confirmStart();await act(async()=>{button('Da, trajno zatvori nalog').props.onPress();});mockOwner={user:{id:A},accountRevision:3};await act(async()=>{resolve();});expect(mockStart).not.toHaveBeenCalled();});
