...
/**
 * What a person reads to recognise the application: status, Task, where and when, the offer and the
 * people. Data to pixels only, memoised on the row alone, so the route's fresh per-render handlers
 * (which the card must keep — a handle captured under one account revision must not act under the
 * next) re-render the thin interactive shell and not this text.
 */
const ApplicationSummary = memo(function ApplicationSummary({ row: p }: { row: MojaPrijavaProjekcija }) {
  return <>
