...
/** A day's row height: the whole cell is the touch target (48), as wide as a seventh of the grid. */
const CELL = 48;
/** The filled circle of a chosen end, inside its cell. */
const DOT = 40;
 ...
 * (an earlier day starts it again); the days between are shaded light green and both ends are filled green. Nothing here
 * decides a result: the caller owns the range and says how many tasks it leaves.
 */
export function DateRangeGrid({ today, from, to, now, onDay }: {
  ...
  return <View style={s.grid}>
    <View style={s.head}>
      ...
      <T accessibilityRole="header" style={s.month}>{monthTitle(month)}</T>
    ...
    <View style={s.week} accessibilityElementsHidden importantForAccessibility="no-hide-descendants">
      {weekdays.map(day => <T key={day.short} style={s.weekday}>{day.short}</T>)}
    </View>
    {weeks.map(week => <View key={week.find(Boolean) ?? 'x'} style={s.week}>
      {week.map((day, index) => day === null ? <View key={`blank-${index}`} style={s.cell} />
        : <Day key={day} day={day} today={today} from={from} to={to} now={now} onDay={onDay} />)}
    </View>)}
  </View>;
}

function Day({ day, today, from, to, now, onDay }: { day: string; today: string; from: string | null; to: string | null; now: Date; onDay: (day: string) => void }) {
  ...
  return <Press accessibilityRole="button" accessibilityLabel={`${dayHeading(day, now)}${day === today ? ', danas' : ''}${past ? ', prošao dan' : ''}`}
    ...
    {band ? <View testID="range-band" style={[s.band, band]} /> : null}
    <View style={[s.dot, (start || end) && s.dotChosen]}>
      <T style={[s.number, day === today && s.today, past && s.past, (start || end) && s.onGreen]}>{Number(day.slice(8, 10))}</T>
    </View>
  </Press>;
}
...
const s = StyleSheet.create({
  ...
  month: { ...sys.type.bodyStrong, color: sys.color.ink, flex: 1, textAlign: 'center' },
  week: { flexDirection: 'row' },
  weekday: { ...sys.type.meta, flex: 1, textAlign: 'center', color: sys.color.muted, paddingVertical: sys.space.xs },
  cell: { flex: 1, height: CELL, alignItems: 'center', justifyContent: 'center' },
  band: { position: 'absolute', top: (CELL - DOT) / 2, height: DOT, backgroundColor: sys.color.greenSoft },
  bandFull: { left: 0, right: 0 },
  bandRight: { left: '50%', right: 0 },
  bandLeft: { left: 0, right: '50%' },
  dot: { width: DOT, height: DOT, borderRadius: sys.radius.pill, alignItems: 'center', justifyContent: 'center' },
  dotChosen: { backgroundColor: sys.color.green },
  number: { ...sys.type.body, color: sys.color.ink, fontVariant: ['tabular-nums'] },
  today: { fontWeight: '700' },
  past: { color: sys.color.muted },
  onGreen: { color: sys.color.onGreen, fontWeight: '700' },
});
