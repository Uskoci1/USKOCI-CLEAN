...
export function WorkerAiCard({profile,compact,review,disabled}:{profile:WorkerAiProfile;compact:boolean;review:()=>void;disabled:boolean}){
  ...
  return <View style={[s.card,compact&&s.cardCompact]}>
    ...
    <T variant="body" style={s.ink}>{profile.location.city||'Područje nije navedeno'}{profile.location.operatingCountryCode?` · ${profile.location.operatingCountryCode}`:''} · {osoba(profile.teamCapacity)}</T>
...
export function WorkerAiReviewDetails({review}:{review:WorkerAiReview}){
  ...
  return <>
    ...
    <View style={s.section}>
      ...
      {/* The zone is named only when it is not Serbian time; a raw zone name is never the only thing said about time. */}
      {p.availability.timezone!=='Europe/Belgrade'?<T variant="meta" tone="muted">Vremenska zona: {p.availability.timezone}</T>:null}
    </View>
...
const s=StyleSheet.create({
  ...
  section:{...card,gap:12},
  // No hairlines inside a card: the section's gap separates the rows.
  row:{gap:3},
