-- USKOČI PKG-014B / owner decision 2026-09-17 point 4
-- SOURCE CANDIDATE ONLY. This file is intentionally outside supabase/migrations.
--
-- Purpose: record what the provider actually reported for each real Gemini call, so the
-- AI test budget can be reported in real tokens instead of bounds. Until now nothing read
-- usageMetadata, so input tokens, output tokens and real cost were unknowable after the
-- fact, and the internal cap, which is a call counter, was the only number available.
--
-- Deliberately ADDITIVE. private.ai_test_budget_v5 and private.ai_test_reservations_v5 are
-- part of the verified budget engine and are NOT altered: no column is added to them, no
-- function of theirs is replaced, and the reservation ceiling is NOT raised. Usage is
-- recorded in its own table, keyed by the same operation id, so accounting can never change
-- an admission decision.
--
-- Usage is accounting, never authorization. Nothing here grants, refunds or releases a
-- reservation, and a missing usage row never blocks a turn.
...
-- One row per provider call that actually reported usage. The operation id is the same
-- client request id the reservation carries, so the two join without touching either.
...
-- Service-only recorder. First write wins: a replay with identical counts is a no-op, and
-- a replay with different counts is refused rather than silently overwriting what the
-- provider first reported.
...
begin
  ...
  -- Usage may only be recorded for a call that was actually admitted. This reads the
  -- reservation; it never writes to it.
...
-- The report the owner asked for, in one authoritative read. Every field is either a real
-- number or an explicit null that the caller must render as UNKNOWN.
...
begin
  ...
  return jsonb_build_object(
    ...
    -- Null means UNKNOWN and must be rendered as UNKNOWN, never as zero. Calls made before
    -- usage capture existed can never be filled in.
    ...
    -- Real spend stays UNKNOWN here on purpose: this database holds no price table, and the
    -- provider's own billing is the only authority for money.
    'realSpendUsd', null,
...
begin
  ...
  -- The verified budget engine must be untouched: the ceiling is still the reviewed
  -- constant and the reservation function still exists exactly as it was.
