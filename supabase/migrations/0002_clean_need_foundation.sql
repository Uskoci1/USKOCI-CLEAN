-- 02 — Potreba
--
-- KEEP iz donora, uz jednu izmenu: iz status enuma je uklonjen
-- AGREEMENT_PENDING. To je ostatak lanca sa trećom potvrdom koji M02/M10
-- ukidaju — izbor atomski pravi CONFIRMED Dogovor.
--
-- Geografija je namerno gruba: numeric(6,2)/(7,2). Tačna adresa NIKAD ne stoji
-- ovde nego u need_sensitive, iza posebne dozvole.
...
create table if not exists public.needs (
  ...
  -- Izbor se vezuje za TAČNU reviziju. Materijalna izmena je diže.
...
-- Tačna adresa i uputstva — odvojeno od javne Potrebe.
...
create table if not exists public.need_selections (
  ...
  -- Idempotencija: isti zahtev ne sme da napravi dva izbora.
