  ...
  with candidate_rows as (
    ...
    select
      ...
      case
        ...
        when v_need_closed(v_need_status => v_need.status) then 'CLOSED'
        when v_need_deadline_expired(v_deadline => v_need.response_deadline) then 'CLOSED'
...
-- Local immutable helpers keep the projection readable without changing any
-- existing lifecycle authority. They are private and not API-executable.
create or replace function private.v_need_closed(v_need_status text)
returns boolean
language sql
immutable
set search_path to 'pg_catalog'
as $$
  select coalesce(v_need_status,'') not in ('PUBLISHED','SELECTION')
$$;

create or replace function private.v_need_deadline_expired(v_deadline timestamptz)
returns boolean
language sql
stable
set search_path to 'pg_catalog'
as $$
  select v_deadline is not null and v_deadline <= statement_timestamp()
$$;

-- Recreate projection after helpers so function dependency is explicit.
create or replace function public.rpc_list_need_candidates(p_need_id uuid)
returns jsonb
language plpgsql
stable
...
begin
  ...
  select n.* into v_need from public.needs n where n.id = p_need_id;
  ...
  from public.need_selections s
  where s.need_id = p_need_id and s.status = 'SELECTED';
  v_remaining_slots := greatest(coalesce(v_need.required_slots,1) - v_selected_slots,0);

  with candidate_rows as (
    select r.id response_id,r.worker_profile_id,r.status raw_response_status,
           r.current_version,r.submitted_against_need_revision,r.submitted_at,
           v.need_revision version_need_revision,v.content_hash,v.price_rsd,
           v.covered_slots,v.proposed_start_at,v.proposed_end_at,v.scope_note,
           snap.snapshot_schema,snap.worker_team_capacity,snap.worker_skills,
           snap.worker_tools,snap.worker_licenses,snap.worker_vehicles,
           public.rpc_get_public_profile(r.worker_profile_id) public_profile,
           private.match_detail(p_need_id,r.worker_profile_id) match_detail,
           exists(select 1 from public.agreements a where a.selected_response_id=r.id) has_agreement
    from public.marketplace_responses r
    left join public.marketplace_response_versions v
      on v.response_id=r.id and v.version=r.current_version
    left join private.response_application_snapshots snap
      on snap.response_id=r.id and snap.response_version=r.current_version
    where r.need_id=p_need_id and r.status<>'DRAFT'
  ), classified as (
    select c.*,
      case
        when c.has_agreement or c.raw_response_status='SELECTED' then 'SELECTED'
        when c.raw_response_status='WITHDRAWN' then 'WITHDRAWN'
        when c.raw_response_status in ('NOT_SELECTED','EXPIRED') then 'CLOSED'
        when private.v_need_closed(v_need.status) then 'CLOSED'
        when private.v_need_deadline_expired(v_need.response_deadline) then 'CLOSED'
        when c.raw_response_status in ('STALE','STALE_REVIEW_REQUIRED')
          or c.current_version is null or c.content_hash is null
          or c.version_need_revision is distinct from v_need.revision
          or c.submitted_against_need_revision is distinct from v_need.revision
          or coalesce((c.match_detail->>'responseAllowed')::boolean,false) is not true
          then 'STALE'
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
      'responseId',c.response_id,'workerProfileId',c.worker_profile_id,
      'needRevision',v_need.revision,'responseNeedRevision',c.version_need_revision,
      'version',c.current_version,'contentHash',c.content_hash,
      'state',c.candidate_state,'canSelect',c.candidate_state='SELECTABLE',
      'priceRsd',c.price_rsd,'coveredSlots',c.covered_slots,'remainingSlots',v_remaining_slots,
      'proposedStartAt',c.proposed_start_at,'proposedEndAt',c.proposed_end_at,
      'scopeNote',coalesce(c.scope_note,''),'publicProfile',c.public_profile,
      'applicationEvidence',case when c.snapshot_schema is null then jsonb_build_object(
        'schema','LEGACY_UNPROVEN','teamCapacity',null,'skills',null,'tools',null,'licenses',null,'vehicles',null)
      else jsonb_build_object('schema',c.snapshot_schema,'teamCapacity',c.worker_team_capacity,
        'skills',to_jsonb(c.worker_skills),'tools',to_jsonb(c.worker_tools),
        'licenses',to_jsonb(c.worker_licenses),'vehicles',to_jsonb(c.worker_vehicles)) end
    ) order by c.submitted_at asc nulls last,c.response_id),'[]'::jsonb)
  into v_result from classified c;
  return v_result;
end;
$function$;

revoke all on function private.v_need_closed(text) from public,anon,authenticated,service_role;
revoke all on function private.v_need_deadline_expired(timestamptz) from public,anon,authenticated,service_role;
revoke all on function public.rpc_list_need_candidates(uuid) from public,anon,authenticated,service_role;
...
begin
  ...
  if has_function_privilege('authenticated','private.v_need_closed(text)','EXECUTE')
     or has_function_privilege('authenticated','private.v_need_deadline_expired(timestamptz)','EXECUTE') then
    raise exception 'RU5_P0D01_POSTCONDITION_FAILED: private helper exposure';
  end if;
