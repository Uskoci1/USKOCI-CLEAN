...
test('full source106 is admitted before original P2 assertions at source103 and delivery104', () => {
  ...
  assert.deepEqual(boundary.deferredSuccessors.map(x => x.file), [retentionExecutionForward,ownedIntakeForward]);
...
test('missing, extra, unknown and reordered successors cannot evade intake invariants', () => {
  ...
  for (const count of [103, 104, 105, 107]) assert.throws(() => deliveryBoundary({ ...plan, source_migration_count: count }));
