...
import {ownedIntakeForward,pushTransportForward,dispatchLockForward,agreementChangeForward} from '../../../scripts/ci/owned-intake-source.mjs';
...
test('source109 is fully admitted while D0140 stays at102 and W05 applies only103',()=>{
  ...
  assert.deepEqual(result.deferredSuccessors.map(x=>x.file),[exportDeliveryForward,retentionExecutionForward,ownedIntakeForward,pushTransportForward,dispatchLockForward,agreementChangeForward]);
...
test('unknown or additional authority successors cannot silently evade old invariants',()=>{
  ...
  for(const count of [102,103,104,105,106,107,108,110])assert.throws(()=>publicationBoundary({...plan,source_migration_count:count}));
...
for(const forward of [exportDeliveryForward,retentionExecutionForward,ownedIntakeForward,pushTransportForward,dispatchLockForward,agreementChangeForward])
for(const mutation of ['missing','changed','unknown'])test('full admission rejects '+mutation+' '+forward+' before the W05 boundary',()=>{
  ...
  try{
    ...
    assert.equal(publicationBoundary(readD0140aPredecessorPlan(root)).fullPlan.source_migration_count,109);
