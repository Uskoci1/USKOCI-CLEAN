...
const denied=async(request,code)=>{const r=await request;assert.ok(r.error,'EXPECTED_DENIAL');if(code)assert.equal(r.error.message,code);};
...
try {
 ...
 await denied(A.client.from('worker_match_preferences').delete().eq('worker_profile_id',A.profile.id),'WORKER_PREFERENCES_REQUIRE_AUTHORITY');
