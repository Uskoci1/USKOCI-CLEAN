...
cp "$GITHUB_WORKSPACE/supabase/proofs/ru2_predecessor_bootstrap.sql" \
  supabase/migrations/20260825000000_ru5_device_live_bootstrap.sql
...
psql "$DB_URL" -v ON_ERROR_STOP=1 -c "delete from supabase_migrations.schema_migrations where version='20260825000000';" >/dev/null
...
apply_live_alias() {
  ...
  psql "$DB_URL" -v ON_ERROR_STOP=1 -f "$GITHUB_WORKSPACE/supabase/migrations/$source_file" >/tmp/ru5-device-last-apply.log
...
# Frozen safety preflight before device fixtures.
...
test "$(psql "$DB_URL" -Atc "select count(*) from private.preselection_qa_answer_versions;")" = "0"
...
: "${LOCAL_API_URL:?local API URL missing}"
: "${LOCAL_ANON_KEY:?local anon/publishable key missing}"
: "${LOCAL_SERVICE_KEY:?local service role/secret key missing}"

{
  echo "RU5_DEVICE_PROOF_DIR=$PROOF_DIR"
  echo "RU5_DEVICE_DB_URL=$DB_URL"
  echo "RU5_DEVICE_SUPABASE_URL=$LOCAL_API_URL"
  echo "RU5_DEVICE_ANON_KEY=$LOCAL_ANON_KEY"
  echo "RU5_DEVICE_SERVICE_ROLE_KEY=$LOCAL_SERVICE_KEY"
} >> "$GITHUB_ENV"

echo "PASS RU5_DEVICE_LIVE79_ENV local_full_stack live79 gated_features_fail_closed"
