...
begin
  ...
  -- Serialize same actor + semantic request key before looking for a receipt.
  ...
  -- DRAFT edits do not currently bump revision. Conservative current-decision
  -- rule: any later decision for this Need/revision invalidates an older one.
  if exists (
    select 1
  ...
  -- Existing needs_enqueue_dispatch trigger must create/update the schedule in
  -- the same transaction. If it does not, abort so PUBLISHED cannot commit alone.
  if not exists (
